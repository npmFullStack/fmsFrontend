...
const BookingTable = ({ 
  ...
  const handleBulkEdit = useCallback(() => {
    if (selected.length === 1) {
      const bookingToEdit = approvedBookings.find(item => item.id === selected[0]);
      if (bookingToEdit && onEdit) {
        onEdit(bookingToEdit);
      }
    } else {
      console.log('Please select only one booking to edit');
      // You can show a toast message here
    }
  }, [selected, approvedBookings, onEdit]);

  const handleBulkDelete = useCallback(() => {
    if (selected.length > 0) {
      const bookingsToDelete = approvedBookings.filter(item => selected.includes(item.id));
      if (onDelete) {
        onDelete(bookingsToDelete);
      }
    }
  }, [selected, approvedBookings, onDelete]);
