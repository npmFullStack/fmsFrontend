// [file name]: CustomerBookingsTable.jsx (Fixed version)
...
const isFullyPaid = (booking) => {
  if (booking.accounts_receivable) {
    ...
    return ar.is_paid === true || (ar.collectible_amount === 0 && !ar.cod_pending);
  }
  return false;
};
...
const getTotalPaymentDue = (booking) => {
  if (booking.accounts_receivable) {
    ...
    // If it's COD pending, return 0 (no pay button)
...
{isApproved && (
  <div className="border-t border-gray-200">
    ...
    {console.log('Booking Action Debug:', {
      ...
      hasAR: !!item.accounts_receivable,
      arData: item.accounts_receivable,
      isCODPending: isCODPending(item),
      isFullyPaid: fullyPaid,
      totalPaymentDue: totalPaymentDue,
      collectibleAmount: item.accounts_receivable?.collectible_amount,
      paymentMethod: item.accounts_receivable?.payment_method,
      isPaid: item.accounts_receivable?.is_paid
    })}
    ...
    /* CASE 3: Has payment due (GCash or new booking without payment method set) */
    totalPaymentDue > 0 ? (
    ...
    /* CASE 4: No payment due but not fully paid or COD pending (shouldn't happen normally) */
    (
      <div className="bg-gray-50 px-4 py-3 flex flex-col sm:flex-row justify-between items-center gap-3">
        <div className="flex items-center gap-2 text-gray-700 mb-3 sm:mb-0">
          <AlertCircle className="w-5 h-5" />
          <div>
            <span className="font-semibold">Payment Status: Pending</span>
            <p className="text-sm text-gray-600">No payment due at this time</p>
          </div>
        </div>
        <button
          onClick={() => handleDownloadStatement(item)}
          className="w-full sm:w-auto bg-gray-600 text-white px-4 py-2 text-sm font-medium rounded-lg hover:bg-gray-700 transition-colors flex items-center justify-center gap-2"
        >
          <Receipt className="w-4 h-4" />
          View Statement
        </button>
