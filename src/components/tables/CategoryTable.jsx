...
const CategoryTable = ({
  ...
  const handleBulkEdit = useCallback(() => {
    if (selected.length === 1) {
      toast('Single item selected. Open edit modal if implemented.');
    } else {
  ...
  const getSortIcon = useCallback((field) => {
    if (sortField !== field) return <ChevronUp className="w-3 h-3 text-gray-400 opacity-50" />;
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ getValue }) => <span className="font-semibold">{toUpperCase(getValue())}</span>,
    },
    {
      ...
      cell: ({ getValue }) => <span className="font-semibold text-success">{formatCurrency(getValue())}</span>,
    },
